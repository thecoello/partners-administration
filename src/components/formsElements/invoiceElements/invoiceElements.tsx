    ...
    return (
      <>
        {model?.company_name && model?.address && model?.zip && model?.country ? <div className='card mb-4 bg-green'>
          <div className='card-body'>
            <div className='mb-3'>
              <label htmlFor='voucher' className='form-label'> <p><b>Upload the proof of payment here (PDF, PNG, JPG or JPEG file) (PDF - 2MB Max)</b></p> </label>
              <input onChange={(e:any) => {
                ...
                if (!["PDF","PNG","JPG","JPEG"].includes(fileExt)) {
                  e.target.value = ''
                  alert('The proof of payment file must be a PDF, PNG, JPG or JPEG file.')
                }

                if(Math.round(e.target.files[0].size / 1024) > 2000){
                  e.target.value = ''
                  alert('The proof of payment file size should not exceed 2MB')
                }

              }} type='file' className='form-control' name='voucher' id='voucher' aria-describedby='voucher' />
            </div>
          </div>
        </div> : null}
